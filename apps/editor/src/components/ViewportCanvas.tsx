...
export function ViewportCanvas() {
  ...
  // Dev-only console handle for manual smoke tests and the perf harness
  // (utils/perfHarness.ts) — the THREE graph is inspected from the devtools
  // console, see main.tsx. Set in an effect so it always points at the
  // committed managers, not a render pass StrictMode discarded. The refs are
  // stable, so readers always reach the live camera/controls through them.
  useEffect(() => {
    if (import.meta.env.DEV) {
      (window as unknown as Record<string, unknown>).__libre3dViewport = {
        sceneManager,
        objectManager,
        cameraRef,
        orbitControlsRef,
  ...
  }, [sceneManager, objectManager, cameraRef, orbitControlsRef]);
